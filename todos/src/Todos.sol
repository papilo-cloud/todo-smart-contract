...
contract Todos {
    ...
    TodoItem public EMPTY_TODOS = TodoItem(0, "", "", false, 0x0000000000000000000000000000000000000000);
    ...
    function deleteTodo(uint256 _todoId) public {
        ...
        require(
            (
                !compareStringsbyBytes(todoItem.todoTitle, EMPTY_TODOS.todoTitle)
                    && !compareStringsbyBytes(todoItem.todoDescription, EMPTY_TODOS.todoDescription)
            ),
            "Todo is already empty"
        );

        todos[_todoId] = EMPTY_TODOS;
    ...
    function getTodosByOwner(address _owner) public view returns (TodoItem[] memory) {
        TodoItem[] memory todoItem = ownerToTodos[_owner];
        return todoItem;
    }
}
